...
# by default, with the root login the mysql database, backup and backup / $DATE/the mysql_ $DATE
DBName=(qiandaonet,unifgroup_sms,unifgroup_cms,unifgroup_pms,unifgroup_dubbo)
DBUser=root
DBPasswd="gfds@#$^9876POIU"
DAY=$(date "+%Y%m%d%H")
TIME=$(date "+%H%M%S")
DESDIR=/mnt/mysql/mysqlback/$DAY
LogFile=/mnt/mysql/mysqlback/run.log
#Setting End
echo "-------------------------------------------" >> $LogFile
